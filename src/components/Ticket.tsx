...
function Ticket({data}: TicketComponentProps) {
  ...
  return (
    <div className="Ticket" onMouseEnter={()=>setShowControls(true)} onMouseLeave={()=>setShowControls(false)}>
      ...
      <div className={"ticketContent"+(isDone?" strike":"")}>{isEdit?<input onKeyDown={OnPressEnterToSave} value={content} autoFocus onChange={e=>SetContent(e.target.value)}/>:content}</div>
