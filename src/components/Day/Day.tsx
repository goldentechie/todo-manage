...
function Day({ date }: DayComponentProps) {
  ...
  const handleESCAndEnter = (event: React.KeyboardEvent) => {
    ...
    if (event.key === 'Enter') {
      if (newTodoContent.trim().length > 0) dispatch(createTodo({date: date.toISOString(),content: newTodoContent.trim(),isDone: false,poolId:date.toISOString(), poolType: "day"}))
      else disableInput();
      setNewTodoContent("");
    }
  }
  return (
    <Droppable id={date.toDateString()} classNames={className}>
